...
# - start logger:
# The colors portion of this file is from:
...
# BEGIN CORG CUSTOMIZATION

# Associate array for storing log colors
typeset -A log_level_colors

warn_color="yellow"
error_color="red"
ok_color="green"
info_color="cyan"
announce_color="magenta"

log_level_colors+=(["info"]="${fg_bold[$info_color]}")
log_level_colors+=(["error"]="${fg_bold[$error_color]}")
log_level_colors+=(["ok"]="${fg_bold[$ok_color]}")
log_level_colors+=(["warn"]="${fg_bold[$warn_color]}")
log_level_colors+=(["announce"]="${fg_bold[$announce_color]}")

# Logs messages, conumsed by the other logger shortcuts
function corg_log() {
	# Fucking unreadable baby, just how I like it.
	status_level=$1
	status_message="${reset_color}[${log_level_colors[$status_level]}$status_level${reset_color}]"
	echo "${status_message} $2"
}

function corg_announce() {
  msg="${reset_color}${log_level_colors[info]}$1"
	corg_log "announce" $msg
}

function corg_debug() {
  if [ "${CORG_LOG_LEVEL}" = 'debug' ]; then
    corg_log "info" $1
  fi
}
